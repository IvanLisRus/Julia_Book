...
function closefigure(x...)
    ...
    if gnuplot_state.current != 0
        for i in gnuplot_state.figs
            handles = [handles; i.handle]  #UPD# ,
        end
    end
    if in(h,handles)       #UPD# contains(arr,i) -> in(i,arr)
...
function figure(h::Int,redraw::Bool)
    ...
    handles = []
    for i in gnuplot_state.figs
        handles = [handles; i.handle]   #UPD# , -> ; everywhere
    end
    ...
    if !in(h,handles)
        gnuplot_state.figs = [gnuplot_state.figs; Figure(h)]   #UPD# , -> ;
    else
        if redraw
            llplot()
        end
