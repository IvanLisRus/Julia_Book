...
function addcoords(x::Coord,y::Coord,Z::Array,conf::CurveConf)
    ...
    # check types
    if nex
        @assert eltype(x)<:Real "Invalid coordinates"  #UPD# replaced assert -> @assert
    end
    if ney
    ...
    elseif isa(x,Range) || isa(x,Range)   #UPD# replaced Range1 -> Range everywhere!!?
        x = collect(x)   #UPD# [x]
    end
    if isa(y,Matrix)
        s = size(y)
    ...
    elseif isa(y,Range) || isa(y,Range)
        y = collect(y)   #UPD# [y]
    end
    ...
    if fig.isempty == true
        ...
        fig.curves = [fig.curves; CurveData(x,y,Z,conf)]   #UPD# , -> ;
    end
...
function adderror(yl::Coord,yh::Coord)
    ...
    if isempty(yl)
        ...
        elseif isa(yl,Range) || isa(yl,Range)
            yl = [yl]
        end
    end
    if !isempty(yh)
        ...
        elseif isa(yh,Range) || isa(yh,Range)
            yh = [yh]
        end
    end
