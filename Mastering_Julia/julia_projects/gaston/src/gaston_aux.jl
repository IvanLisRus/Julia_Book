...
function gnuplot_init()
	...
	#UPD#
    f = @linux ? ccall(:popen, Ptr{Int}, (Ptr{UInt8},Ptr{UInt8}), "gnuplot" ,"w") : ccall( (:_popen, "msvcr100"), Ptr{Int}, (Ptr{UInt8},Ptr{UInt8}), "gnuplot", "w")
                # ccall(:_popen, Ptr{Int}, (Ptr{UInt8},Ptr{UInt8}), "gnuplot" ,"w")
    #eof UPD#
...
function gnuplot_exit(x...)
    ...
    if gnuplot_state.running
        ...
        #UPD#
        err = @linux ? ccall(:pclose, Int, (Ptr{Int},), gnuplot_state.fid) : ccall((:_pclose, "msvcr100"), Int, (Ptr,), gnuplot_state.fid) # windows
        ##rm(gnuplot_state.tmpdir; force=true, recursive=true)
        #eof UPD#
...
function histdata(s,bins)
    ...
    ms = minimum(s)-eps()  #UPD# min -> minimum
    Ms = maximum(s)        #UPD# max -> maximum
    delta = (Ms-ms)/bins
    x = ms:delta:Ms
    y = zeros(length(x))
